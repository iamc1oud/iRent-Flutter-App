import 'package:flutter/material.dart';

class AppStyle {
  final Color secondaryTextColor = Colors.indigo;
  final Color headingBackgroundColor = Colors.indigo[400];
  final Color primaryTextColor = Colors.white;
}

class AppTextStyle {
  final TextStyle headingStyle = TextStyle(color: Colors.white, fontSize: 18, fontFamily: "RobotoSlab");
  final TextStyle headingSecondaryStyle = TextStyle(color: Colors.white70, fontSize: 12, fontFamily: "RobotoSlab");
  final TextStyle cardHeadingPrimaryStyle = TextStyle(fontSize: 18, fontFamily: "RobotoSlab", color: Colors.white);
  final TextStyle whiteTextColor = TextStyle(color: Colors.white);
}
